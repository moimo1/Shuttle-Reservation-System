...
export const getShuttles = async (req, res) => {
    try {
        ...
        const reservations = await Reservation.find({ status: "active" }, "shuttle seatNumber");
        const takenMap = {};
        reservations.forEach((r) => {
            ...
            if (!isNaN(seatNum) && seatNum >= 1 && seatNum <= TOTAL_SEATS) {
                if (!takenMap[shuttleId])
                    takenMap[shuttleId] = [];
                takenMap[shuttleId].push(seatNum);
            }
        });
        const enriched = shuttles.map((s) => {
...
export const reserveShuttle = async (req, res) => {
    try {
...
//# sourceMappingURL=shuttleController.js.map